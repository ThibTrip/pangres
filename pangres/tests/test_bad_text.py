...
def test_bad_text_insert(engine, schema):
    ...
    upsert(engine=engine, schema=schema, table_name=TableNames.BAD_TEXT, df=df_test,
...
def test_bad_column_names(engine, schema, iteration):
    ...
    upsert(engine=engine, schema=schema, df=df_test, table_name=TableNames.BAD_COLUMN_NAMES, if_row_exists='update')
...
def test_bad_column_name_postgres_raises(engine, schema):
    ...
    with pytest.raises(BadColumnNamesException) as exc_info:
        upsert(engine=engine, schema=schema, df=df, table_name=TableNames.BAD_COLUMN_NAMES, if_row_exists='update')
...
def test_column_named_values(engine, schema):
    ...
    upsert(engine=engine, schema=schema, df=df, if_row_exists='update', table_name=TableNames.COLUMN_NAMED_VALUES)
